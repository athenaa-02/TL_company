
import AboutTlBox from "./AboutTlBox";

function AboutTl() {
  return (
    <>
      <main className="bg-white pt-24 text-text-basic">
        <section className="relative flex min-h-[calc(100vh-6rem)] justify-center overflow-hidden px-6 py-24 text-center sm:px-10 lg:px-16">
          <div className="absolute bottom-0 left-[12%] top-0 w-2 bg-accent" />
          <div className="relative z-10 w-full max-w-235">
            <h1 className="font-volkhov text-4xl font-bold leading-normal text-heading sm:text-5xl">
              TL Company
            </h1>
            <p className="mt-5 text-base leading-8 text-text-basic">
              founded in 2019
            </p>
            <div className="mx-auto mt-8 max-w-235 text-base leading-8 text-text-basic sm:text-lg">
              <p>
                We ensure that you&apos;ll embark on a perfectly planned, safe
                vacation at a price you can afford.
              </p>
              <p>
                We ensure that you&apos;ll embark on a perfectly planned, safe
                vacation at a price you can afford.
              </p>
              <p>
                We ensure that you&apos;ll embark on a perfectly planned, safe
                vacation at a price you can afford.
              </p>
              <p>
                We ensure that you&apos;ll embark on a perfectly planned, safe
                vacation at a price you can afford.
              </p>
              <p>
                We ensure that you&apos;ll embark on a perfectly planned, safe
                vacation at a price you can afford.
              </p>
              <p>sdfkhdvnsvbxdhbvhsd</p>
            </div>
          </div>
        </section>
        <AboutTlBox />
      </main>
    </>
  );
}

export default AboutTl;
